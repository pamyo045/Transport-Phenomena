function concentric_cylinder_steady_momentum       
    % parameter values
    omega_o = 2;
    omega_i = 4;
    Ro      = 0.8;
    ...
    function createGUI
        ...
        xlabel('Radial distance, r (m) ','FontSize',16);
        ylabel('v_{\theta}(r), (m/s) ','FontSize',16);
        ylim([0,YMax]);
        xlim([0,XMax]);
        plotXRange = hPlot1.XData(end) - hPlot1.XData(1);
        plotYRange = hPlot1.YData(end) - hPlot1.YData(1);
        plotXMid   = hPlot1.XData(1) + plotXRange/2;
        ptxt1 = text(plotXMid,0.9*max(hPlot1.YData),'$v_{\theta}(r) = \frac{kR}{1-k^2}[(\Omega_o-\Omega_ik^2)(\frac{r}{kR})+(\Omega_i-\Omega_o)(\frac{kR}{r})$','interpreter','latex','FontSize',14);
        ptxt1.FontSize = 15;
